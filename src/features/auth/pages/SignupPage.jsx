...
export default function SignupPage() {
  ...
  const initialRole = initialEmail ? "patient" : (userType === "therapist" ? "therapist" : "patient");
  ...
  const submit = async (e) => {
    ...
    try {
      // Sending name as fullName. Adjust if backend expects 'fullName' key.
      await register({ full_name: fullName, email, password, role });
      
      // Show success message (using alert for now as toast is not fully integrated in this file)
        ...
        <div className="w-full max-w-6xl flex flex-col md:flex-row items-stretch gap-8">
            ...
            <div className="relative h-full min-h-[480px]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent flex items-end">
                <div className="p-8 text-white max-w-sm">
                  <h2 className="text-3xl font-bold">{t('auth.transformExperience')}</h2>
                  <p className="mt-2 text-sm">{t('auth.transformDesc')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 flex items-center justify-center">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 w-full max-w-md">
              <h1 className="text-3xl font-bold text-text-primary dark:text-white mb-2">{t('auth.joinToday')}</h1>
              <p className="text-text-muted dark:text-slate-400 mb-6">{t('auth.joinSubtitle')}</p>
              ...
              <button
                ...
                onClick={() => alert("Sign up with Google placeholder")}
              >
                <img src="https://www.svgrepo.com/show/355037/google.svg" alt="Google" className="w-5 h-5" />
                <span className="text-sm text-text-body dark:text-slate-300">{t('auth.signUpWith')}</span>
              </button>
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <div>
                  <label className="block text-text-body dark:text-slate-300 text-sm mb-1">{t('auth.fullName')}</label>
                  <input
                    value={fullName}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-border dark:border-slate-600 rounded-md px-3 py-2 bg-white dark:bg-slate-700 text-text-primary dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-cyan-400 transition disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-100 dark:disabled:bg-slate-800"
                  />
                </div>
                ...
                <div>
                  <label className="block text-text-body dark:text-slate-300 text-sm mb-2">{t('auth.role')}</label>
                  <div className="flex gap-6">
                    <label className="flex items-center gap-2 text-text-body dark:text-slate-300">
                      <input
                        ...
                        disabled={!!initialEmail}
                        className="accent-primary dark:accent-cyan-400 disabled:opacity-50 disabled:cursor-not-allowed"
                      />
                      {t('auth.patient')}
                    </label>
                    <label className={`flex items-center gap-2 text-text-body dark:text-slate-300 ${!!initialEmail ? 'opacity-50 cursor-not-allowed' : ''}`}>
                      <input
                        type="radio"
                        name="role"
                        value="therapist"
              ...
              <p className="text-center text-text-muted dark:text-slate-400 text-sm mt-4">
                {t('auth.haveAccount')}{" "}
                <button onClick={() => navigate("/login")} className="text-primary dark:text-cyan-400 font-medium hover:underline">
                  {t('auth.login')}
                </button>
              </p>
            </div>
